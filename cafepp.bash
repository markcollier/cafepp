...
#declare -a jsons=("cafepp" "cafepp_experiments" "cafepp_vars_mon" "dfp_csiro-gfdl")
#for json in "${jsons[@]}";do
#awk -f uncomment_json.awk JsonTemplates/${json}.json > ${json}.json
#done
#exit
...
#./cafepp.py -i5 --version v20170726 --initialisation=1 --realisation=$realisation --physics=1 --forcings=1 -v $ivar --ybeg=$ybeg --yend=$yend --ybeg_min=$ybeg_min --yend_max=$yend_max --mbeg=$mbeg_now --mend=$mend_now --mbeg_min=$mbeg_now --mend_max=$mend_now --idir=$idir --season=$season --levs=$levs --cmorlogfile=cmor_log.txt -l stdout.txt --json_input_instructions=cafepp.json
